...
### Save Session
```bash
orchestrator pair --save [--name <name>]
```

### Load Session
```bash
orchestrator pair --load <session-id>
```

### Export Session
```bash
orchestrator pair --export <session-id> [--format json|md]
```
...
```bash
orchestrator pair --start --background
```

### Monitor Background Session
```bash
orchestrator pair --monitor
```

### Attach to Background Session
```bash
orchestrator pair --attach <session-id>
```
...
```bash
orchestrator pair --template refactor
```
...
### Feature Template
```bash
orchestrator pair --template feature
```
- Focus: Implementation
...
### Debug Template
```bash
orchestrator pair --template debug
```
...
### Learning Template
```bash
orchestrator pair --template learn
```
- Mode: Mentor
- Pace: Slow
...
```bash
orchestrator pair --report <session-id>
```
...
```bash
orchestrator pair --list
```

### Switch Between Sessions
```bash
orchestrator pair --switch <session-id>
```

### Merge Sessions
```bash
orchestrator pair --merge <session-1> <session-2>
```
...
### Manual Recovery
```bash
orchestrator pair --recover [--point <timestamp>]
```

### Crash Recovery
```bash
orchestrator pair --crash-recovery
```
...
### Share with Team
```bash
orchestrator pair --share <session-id> \
  --team <team-id>
```

### Export for Review
```bash
orchestrator pair --export-review <session-id>
```

### Create Learning Material
```bash
orchestrator pair --create-tutorial <session-id>
```
...
### Session Recording
```bash
orchestrator pair --start --record
```
Records all interactions for playback.

### Session Replay
```bash
orchestrator pair --replay <session-id>
```
Replay recorded session for learning.

### Session Analytics
```bash
orchestrator pair --analytics <session-id>
```
