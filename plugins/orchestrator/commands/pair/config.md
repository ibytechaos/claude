...
Main configuration file: `.orchestrator/pair-config.json`
...
```bash
# Set single value
orchestrator pair config set defaultMode switch

# Set nested value
orchestrator pair config set verification.threshold 0.98

# Set from file
orchestrator pair config import config.json
```

### Get Configuration
```bash
# Get all configuration
orchestrator pair config get

# Get specific value
orchestrator pair config get defaultMode

# Export configuration
orchestrator pair config export > config.json
```

### Reset Configuration
```bash
# Reset to defaults
orchestrator pair config reset

# Reset specific section
orchestrator pair config reset verification
```
...
### Create Profile
```bash
orchestrator pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
### Use Profile
```bash
orchestrator pair --start --profile refactoring
```

### List Profiles
```bash
orchestrator pair profile list
```
...
### Project-Specific
`.orchestrator/pair-config.json` in project root

### User-Specific
`~/.orchestrator/pair-config.json`

### Global
`/etc/orchestrator/pair-config.json`
...
orchestrator pair config validate

# Test configuration
orchestrator pair config test
```
...
### From Version 1.x
```bash
orchestrator pair config migrate --from 1.x
```

### Export/Import
```bash
# Export current config
orchestrator pair config export > my-config.json

# Import config
orchestrator pair config import my-config.json
```
