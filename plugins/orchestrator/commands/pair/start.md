...
```bash
orchestrator pair --start [options]
```
...
# View session output
orchestrator pair output session_id

# End background session
orchestrator pair --end session_id
```
...
### With Git
```bash
orchestrator pair --start --git --auto-commit
```

### With CI/CD
```bash
orchestrator pair --start --ci --non-interactive
```

### With IDE
```bash
orchestrator pair --start --ide vscode
```
