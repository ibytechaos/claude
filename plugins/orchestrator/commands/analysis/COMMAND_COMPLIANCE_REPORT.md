...
- `mcp__orchestrator__*` tools (preferred)
- `orchestrator` commands (as fallback)
...
- Updated to: `Tool: mcp__orchestrator__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__orchestrator__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__orchestrator__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__orchestrator__*` format
