...
```bash
orchestrator swarm init
```

### Mesh topology for research

```bash
orchestrator swarm init --topology mesh --max-agents 5 --strategy balanced
```

### Hierarchical for development

```bash
orchestrator swarm init --topology hierarchical --max-agents 10 --strategy parallel --auto-spawn
```

### GitHub-focused swarm

```bash
orchestrator swarm init --topology star --github --memory
```
