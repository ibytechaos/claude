...
```bash
orchestrator auto agent [options]
```
...
```bash
orchestrator auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
orchestrator auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
orchestrator auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
orchestrator auto agent -t "Fix bug in login" -s minimal
```
