...
```bash
orchestrator github swarm [options]
```
...
```bash
orchestrator github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
orchestrator github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
orchestrator github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
orchestrator github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
...
```javascript
mcp__orchestrator__github_swarm {
