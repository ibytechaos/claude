...
orchestrator truth --period 7d

# Export to HTML report
orchestrator truth --export report.html --format html
```

### Advanced Analysis
```bash
# Monitor real-time scores
orchestrator truth --watch

# Find problematic files
orchestrator truth --threshold 0.8

# Agent-specific metrics
orchestrator truth --agent coder --period 24h

# JSON for processing
orchestrator truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    orchestrator truth --format json > truth.json
...
orchestrator truth --format json | \
...
Set truth display preferences in `.orchestrator/config.json`:
...
```json
{
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".orchestrator/metrics/truth-daily.json"
    }
  }
}
```
