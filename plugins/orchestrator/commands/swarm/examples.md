...
#### Using CLI (Fallback)
```bash
orchestrator swarm "research AI trends" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel
...
#### Using CLI (Fallback)
```bash
orchestrator swarm "analyze codebase" \
  --strategy analysis \
  --mode mesh \
  --parallel \
  --timeout 300
...
try {
  await mcp__orchestrator__task_orchestrate({
    "task": "complex operation",
    "strategy": "parallel"
  })
} catch (error) {
  // Check swarm health
  const status = await mcp__orchestrator__swarm_status({})
  
  // Log error patterns
  await mcp__orchestrator__error_analysis({
    "logs": [error.message]
  })
}
```
