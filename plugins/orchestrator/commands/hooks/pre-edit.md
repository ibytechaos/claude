...
```bash
orchestrator hook pre-edit [options]
```
...
```bash
orchestrator hook pre-edit --file "src/auth/login.js"
```

### With validation

```bash
orchestrator hook pre-edit -f "config/database.js" --validate-syntax
```

### Manual agent assignment

```bash
orchestrator hook pre-edit -f "api/users.ts" --auto-assign-agent false
```

### Safe editing with backup

```bash
orchestrator hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
orchestrator hook pre-edit --file "path/to/file.js" --validate-syntax
```
