...
```bash
orchestrator hook pre-task [options]
```
...
```bash
orchestrator hook pre-task --description "Implement user authentication"
```

### With memory loading

```bash
orchestrator hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
orchestrator hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
orchestrator hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
orchestrator hook pre-task --description "Your task here"
```
