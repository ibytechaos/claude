...
```bash
orchestrator hook post-edit [options]
```
...
```bash
orchestrator hook post-edit --file "src/components/Button.jsx"
```

### With memory storage

```bash
orchestrator hook post-edit -f "api/auth.js" --memory-key "auth/login-implementation"
```

### Format and validate

```bash
orchestrator hook post-edit -f "config/webpack.js" --auto-format --validate-output
```

### Neural training

```bash
orchestrator hook post-edit -f "utils/helpers.ts" --train-patterns --memory-key "utils/refactor"
```
...
```bash
# After editing files
orchestrator hook post-edit --file "path/to/edited.js" --memory-key "feature/step1"
```
