...
```bash
orchestrator hook post-task [options]
```
...
```bash
orchestrator hook post-task --task-id "auth-implementation"
```

### With full analysis

```bash
orchestrator hook post-task -t "api-refactor" --analyze-performance --generate-report
```

### Memory storage

```bash
orchestrator hook post-task -t "bug-fix-123" --store-decisions --export-learnings
```

### Quick cleanup

```bash
orchestrator hook post-task -t "minor-update" --analyze-performance false
```
...
orchestrator hook post-task --task-id "your-task-id" --analyze-performance true
```
