...
hooks:
  pre: |
    orchestrator hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    orchestrator hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
orchestrator hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
orchestrator memory retrieve --key "project/architecture"
orchestrator memory retrieve --key "project/standards"
```
...
orchestrator memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
orchestrator hooks notify --message "Code analysis complete: ${summary}"
```
