...
orchestrator agent spawn load-balancer --type coordinator

# Start load balancing
orchestrator load-balance --swarm-id <id> --strategy adaptive

# Monitor load distribution
orchestrator agent-metrics --type load-balancer

# Adjust balancing parameters
orchestrator config-manage --action update --config '{"stealThreshold": 5, "agingBoost": 10}'
```

### Performance Monitoring
```bash
# Real-time load monitoring
orchestrator performance-report --format detailed

# Bottleneck analysis
orchestrator bottleneck-analyze --component swarm-coordination

# Resource utilization tracking
orchestrator metrics-collect --components ["load-balancer", "task-queue"]
```
