...
orchestrator health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
orchestrator metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
orchestrator sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
orchestrator alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
orchestrator anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
orchestrator notification-config --channels ["slack", "email", "webhook"]
```
