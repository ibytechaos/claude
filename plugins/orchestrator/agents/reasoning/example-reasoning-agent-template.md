...
```bash
# Command
orchestrator agent run [agent-name] "[task description]" \
  --enable-memory \
...
### With orchestrator CLI:
```bash
orchestrator agent run [agent-name] "[task]" \
  --enable-memory \
  --memory-domain [domain]
```
...
orchestrator agent run [agent-name] "Task 1" --enable-memory
# Success rate: ~60%
...
orchestrator agent memory list --domain [your-domain]

# View statistics
orchestrator agent memory status

# Benchmark performance
orchestrator agent memory benchmark
```
...
- **Available Agents**: `orchestrator agent agents`
...
```bash
orchestrator agent run adaptive-security-auditor \
...
orchestrator agent run adaptive-security-auditor "Audit codebase" --enable-memory
...
orchestrator agent memory list --domain security
```
```
